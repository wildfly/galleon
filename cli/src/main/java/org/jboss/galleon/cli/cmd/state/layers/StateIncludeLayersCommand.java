 ...
 */
@CommandDefinition(name = "include-layers", description = HelpDescriptions.INCLUDE_LAYERS, activator = ConfigDependentCommandActivator.class)
public class StateIncludeLayersCommand extends AbstractLayersCommand {
    ...
    protected void runCommand(PmCommandInvocation invoc, State state) throws IOException, ProvisioningException, CommandExecutionException {
        try {
            state.includeLayersConfiguration(invoc.getPmSession(), getConfiguration(state), layers.split(",+"));
        } catch (Exception ex) {
