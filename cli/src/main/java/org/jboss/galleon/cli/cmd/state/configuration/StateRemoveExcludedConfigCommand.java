 ...
 */
@CommandDefinition(name = "remove-excluded", description = "Remove an already excluded configuration", activator = ProvisionedDefaultConfigCommandActivator.class)
public class StateRemoveExcludedConfigCommand extends AbstractProvisionedDefaultConfigCommand {
    ...
    protected void runCommand(PmCommandInvocation invoc, State session, FeaturePackConfig config) throws IOException, ProvisioningException, CommandExecutionException {
        try {
            session.removeExcludedConfiguration(invoc.getPmSession(), ConfigurationUtil.getExcludedConfigurations(invoc.getPmSession(), config, getConfiguration()));
        } catch (Exception ex) {
