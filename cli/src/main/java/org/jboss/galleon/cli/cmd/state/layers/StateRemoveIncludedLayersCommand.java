 ...
 */
@CommandDefinition(name = "remove-included-layers", description = HelpDescriptions.REMOVE_INCLUDED_LAYERS, activator = ConfigDependentCommandActivator.class)
public class StateRemoveIncludedLayersCommand extends AbstractLayersCommand {
    ...
    protected void runCommand(PmCommandInvocation invoc, State state) throws IOException, ProvisioningException, CommandExecutionException {
        try {
            state.removeIncludedLayersConfiguration(invoc.getPmSession(), getConfiguration(state), layers.split(",+"));
        } catch (Exception ex) {
