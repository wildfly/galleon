 ...
 */
public class CoreStateRemoveExcludedLayersCommand extends CoreAbstractLayersCommand<StateRemoveExcludedLayersCommand> {
    ...
    @Override
    protected void runCommand(ProvisioningSession session, State state, StateRemoveExcludedLayersCommand command) throws IOException, ProvisioningException, CommandExecutionException {
        try {
            state.removeExcludedLayersConfiguration(session, getConfiguration(state, command), command.getLayers().split(",+"));
        } catch (Exception ex) {
