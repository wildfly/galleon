 ...
 */
public class CoreStateRemoveIncludedPackageCommand extends CoreAbstractProvisionedPackageCommand {
    ...
    protected void runCommand(ProvisioningSession session, State state, FeaturePackConfig config, AbstractProvisionedPackageCommand command) throws IOException, ProvisioningException, CommandExecutionException {
        try {
            state.removeIncludedPackage(session, PackagesUtil.getIncludedPackages(session, config, command.getPackage()));
        } catch (Exception ex) {
