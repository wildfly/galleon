...
public class StateInfoUtil {
    ...
    public static String buildDependencies(List<FeaturePackLocation> dependencies, Map<FPID, FeaturePackConfig> configs) {
        if (!dependencies.isEmpty()) {
            ...
            for (FeaturePackLocation d : dependencies) {
                ...
                line.add(new Cell(Util.formatChannel(d)));
    ...
    public static String buildPatches(ProvisioningSession session, PmCommandInvocation invoc, ProvisioningLayout<FeaturePackLayout> layout) throws ProvisioningException {
        ...
        for (FeaturePackLayout fpLayout : layout.getOrderedFeaturePacks()) {
            ...
            for (FeaturePackLayout patch : patches) {
                FeaturePackLocation loc = session.getExposedLocation(null, patch.getFPID().getLocation());
    ...
    public static void printFeaturePack(ProvisioningSession session, PmCommandInvocation commandInvocation, FeaturePackLocation loc) {
        loc = session.getExposedLocation(null, loc);
        Table t = new Table(Headers.PRODUCT, Headers.BUILD, Headers.UPDATE_CHANNEL);
        t.addLine(loc.getProducer().getName(), loc.getBuild(), Util.formatChannel(loc));
        commandInvocation.print(t.build());
    }

    private static String buildFeaturePacks(ProvisioningSession session, PmCommandInvocation commandInvocation,
        ...
        for (FeaturePackConfig c : fps) {
            FeaturePackLocation loc = session.getExposedLocation(installation, c.getLocation());
            List<Cell> line = new ArrayList<>();
            ...
            line.add(new Cell(Util.formatChannel(loc)));
            t.addCellsLine(line);
        }
        if (!t.isEmpty()) {
    ...
    public static void displayInfo(ProvisioningSession session, PmCommandInvocation invoc, Path installation,
        ...
        try {
            ...
            invoc.println("");
            displayFeaturePacks(session, invoc, installation, config);

            if (type != null) {
                invoc.println("");
                try (ProvisioningLayout<FeaturePackLayout> layout = session.getLayoutFactory().newConfigLayout(config)) {
                    switch (type) {
                        case ALL: {
                            FeatureContainer container = supplier.apply(layout);
                            if (displayDependencies(session, invoc, layout)) {
                                invoc.println("");
                            }
                            if (displayPatches(session, invoc, layout)) {
                                invoc.println("");
                            }
                        ...
                        case DEPENDENCIES: {
                            String deps = buildDependencies(session, invoc, layout);
                            if (deps != null) {
                                invoc.print(deps);
                            } else {
                        ...
                        case PATCHES: {
                            String patches = buildPatches(session, invoc, layout);
                            if (patches != null) {
                                invoc.print(patches);
                            } else {
    ...
    private static void displayFeaturePacks(ProvisioningSession session, PmCommandInvocation invoc, Path installation,
            ProvisioningConfig config) {
        String str = buildFeaturePacks(session, invoc, installation, config.getFeaturePackDeps());
        if (str != null) {
            invoc.print(str);
        }
    }

    private static boolean displayDependencies(ProvisioningSession session, PmCommandInvocation invoc, ProvisioningLayout<FeaturePackLayout> layout) throws ProvisioningException {
        String str = buildDependencies(session, invoc, layout);
        if (str != null) {
            invoc.print(str);
        }
        return str != null;
    ...
    private static String buildDependencies(ProvisioningSession session, PmCommandInvocation invoc, ProvisioningLayout<FeaturePackLayout> layout) throws ProvisioningException {
        ...
        for (FeaturePackLayout fpLayout : layout.getOrderedFeaturePacks()) {
            ...
            if (!isProduct) {
                FeaturePackLocation loc = session.getExposedLocation(null, fpLayout.getFPID().getLocation());
    ...
    private static boolean displayPatches(ProvisioningSession session, PmCommandInvocation invoc, ProvisioningLayout<FeaturePackLayout> layout) throws ProvisioningException {
        String str = buildPatches(session, invoc, layout);
        if (str != null) {
            invoc.print(str);
        }
        return str != null;
