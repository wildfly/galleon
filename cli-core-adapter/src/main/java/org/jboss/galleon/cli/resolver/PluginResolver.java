...
public class PluginResolver implements Resolver<ResolvedPlugins> {
    ...
    private PluginResolver(ProvisioningSession session, ProvisioningConfig config) {
    ...
    private PluginResolver(ProvisioningSession session, Path file) {
        Objects.requireNonNull(session);
    ...
    private PluginResolver(ProvisioningSession session, ProvisioningLayout<FeaturePackLayout> layout) {
        Objects.requireNonNull(session);
    ...
    public static PluginResolver newResolver(ProvisioningSession session,
            ProvisioningLayout<FeaturePackLayout> layout) {
        return new PluginResolver(session, layout);
    }

    public static PluginResolver newResolver(ProvisioningSession session, ProvisioningConfig config) {
        return new PluginResolver(session, config);
    }

    public static PluginResolver newResolver(ProvisioningSession session, Path file) {
        return new PluginResolver(session, file);
    }

    public static PluginResolver newResolver(ProvisioningSession session, FeaturePackLocation loc) throws ProvisioningDescriptionException {
