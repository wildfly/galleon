 ...
 * @author jdenise
 */
public class OlderVersionTestCase extends FeaturePackRepoTestBase {
    ...
    public void testOlderVersion() throws Exception {
        ...
        String patched = content.replace("galleon-min-version=\""+ CoreVersion.getVersion() +"\"", "galleon-min-version=\"1.0.0.Final\"");
        ...
        try (Provisioning p = builder.newProvisioningBuilder().setInstallationHome(installHome).build()) {
            config = GalleonProvisioningConfig.builder().addFeaturePackDep(FP1_100_GAV.getLocation()).build();
            p.provision(config);
            config = p.getProvisioningConfig();
        }
        // Retrieve the provisioned config to build a new Provisioning
        try (Provisioning p2 = builder.newProvisioningBuilder(config).setInstallationHome(installHome).build()) {
            assertEquals(config, p2.getProvisioningConfig());
        }

        try (Provisioning p3 = builder.newProvisioningBuilder(PathsUtils.getProvisioningXml(installHome)).setInstallationHome(installHome).build()) {
            assertEquals(config, p3.getProvisioningConfig());
        }
    }
