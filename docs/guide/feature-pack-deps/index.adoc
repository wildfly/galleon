...
----
 <feature-pack xmlns="urn:jboss:galleon:feature-pack:2.0"
    ...
    <dependencies>
        ...
        <dependency location="fp3@maven(org.jboss.universe:community-universe):1#1.0.2" allowedFamily="family1:crit1,crit2"/>
    </dependencies>
    <default-packages>
...
Note that the dependency on `fp3` allows that, at provisioning time, a member of the family `family1` that is the family of the `fp3` feature-pack can be used to replace `fp3`. 
The member of the family must implements the criteria `crit1` and `crit2`.
