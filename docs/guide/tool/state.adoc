...
_[/]$ list-feature-packs_ + 

This command lists all the latest feature packs present in all the installed universes.

You can search for feature-packs that match a given pattern using _--filter-pattern=<pattern>_. +
For example: +
_[/]$ list-feature-packs --filter-pattern=wildfly*_

TIPS: The information displayed by this command can be re-used when installing, updating features-packs.
