...
<feature-pack xmlns="urn:jboss:galleon:feature-pack:3.0"
    ...
    <family name="family1"> <!--2-->
        <criteria name="crit1" inherited="false"/>
        <criteria name="crit2" inherited="true"/>
    </family>
    <dependencies>
        <dependency location="wildfly-core@maven(org.jboss.universe:community-universe):current#6.0.0.Beta1" allowedFamily="family1:crit2"/> <!--3-->
    </dependencies>
    <config model="standalone" name="standalone.xml"> <!--4-->
...
----
<1> Feature-pack location, build info.
<2> Optional family and criteria. A family allows to group compatible feature-packs inside the same family. Family members can then be swapped at provisioning time. Criteria are
expressing what are the feature-pack capabilities. Some can be inherited from dependencies, other are implemented by this feature-pack.
<3> Dependency on another feature-pack. In this case, the `crit2` is inherited.
<4> Description of the default `standalone.xml` configuration
<5> List of packages that should be installed by default
<6> List of provisioning plugins
<7> List of protected system paths
