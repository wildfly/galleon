 ...
 */
public abstract class AbstractDynamicCommand extends MapCommand<PmCommandInvocation> implements GalleonCLIDynamicCommand {
    ...
    private class DynamicOptionsProvider implements MapProcessedOptionProvider {
        ...
        public List<ProcessedOption> getOptions(List<ProcessedOption> currentOptions) {
            ...
            try {
                ...
                String version = getCoreVersion(pmSession);
                GalleonCommandExecutionContext ctx = pmSession.getGalleonContext(version);
                List<DynamicOption> parameters = ctx.getDynamicOptions(AbstractDynamicCommand.this);
    ...
    public CommandResult execute(PmCommandInvocation session) throws CommandException {
        ...
        try {
            ...
            if (getCommandClassName(pmSession) != null) {
                ctx = getGalleonContext(pmSession);
                if (ctx == null) {
                    String version = getCoreVersion(pmSession);
                    ctx = session.getPmSession().getGalleonContext(version);
                }
                ctx.executeDynamic(this, session, options);
            } else {
